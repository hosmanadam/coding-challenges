...
def prod(lst):
  return reduce(lambda x,y: x*y, [num for num in lst])
...
coordinates = [
  { 'x_range': range(len(grid)),
    'y_range': range(len(grid[0])-3),
     'x_step': 0,
     'y_step': 1,},
  { 'x_range': range(len(grid)-3),
...
for c in coordinates:
  for x in c['x_range']:
    for y in c['y_range']:
      product = prod([grid[x+i*c['x_step']][y+i*c['y_step']] for i in range(4)])
      if product > greatest_product_so_far:
        greatest_product_so_far = product
